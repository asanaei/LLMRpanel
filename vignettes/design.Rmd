...
language model personas. Use it to pretest questions, pilot factorial
designs, calculate sample sizes from pilot dispersion, or measure responses
from a configured model. `panel_calibrate()` compares closed-item response
...
2. **Design piloting.** Run vignette and conjoint tasks and estimate response
...
These computations describe the configured model and supplied personas. A
human benchmark provides a separate comparison; it does not turn model
responses into observations from a human population.
...
`panel_administer()` sends each item to each persona. It randomizes item and
option order by default and records `item_position` and `option_order`. The
example repeats the administration with a second model configuration so
response patterns can be compared across models.
...
`resp` has no benchmark attached. `resp_qwen` is a second model run, not a
human benchmark.

## Compare responses with a benchmark

`panel_calibrate()` compares valid model response shares with benchmark
shares for matching item-response pairs. It also records benchmark coverage
and item-level nonresponse. `bench_fund` supplies shares for one closed item.
...
`attr(resp, "calibration")$nonresponse` gives the missing response proportion
for each closed item. The comparison shares use nonmissing responses as their
denominator.

## Factorial stimuli

`vignette_design()` expands a text template over every combination of factor
levels.
...
`conjoint_design()` uses R's random-number generator to construct the design
specification. `panel_administer()` draws the profiles each respondent sees.
Set a seed before administration to reproduce those respondent-level draws.
...
`conjoint_design()` attempts to use distinct profiles within each task and
warns when the attribute space cannot supply them. `conjoint_instrument()`
creates one forced-choice item per task. Administration renders a fresh draw
for each respondent and records it with the response. `amce()` estimates from
those recorded profiles relative to the first level of each attribute and
calculates standard errors clustered by persona.
...
`amce()` returns one row for each observed attribute level. Baseline levels have
estimate 0 and missing standard errors. Other rows contain the estimated
contrast and 95 percent interval.
