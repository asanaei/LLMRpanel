...
resp
LLMR::report(resp)
attr(resp, "benchmark")$nonresponse
```

`attr(resp, "benchmark")$nonresponse` gives the missing response proportion
...
for each respondent and records it with the response. `conjoint_amce()`
estimates from those recorded profiles relative to the first level of each
attribute and calculates standard errors clustered by persona.
...
hosted endpoints change. Each response retains `response_text`, `response_id`,
`success`, `model`, and `provider`, including when a reply cannot be matched to
a closed-item option. `finish_reason` is retained when the runner supplies it.
