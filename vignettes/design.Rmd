...
`conjoint_design()` uses R's random-number generator. Set a seed before the
call to reproduce the task profiles.
...
renders one forced-choice item per task. `amce()` estimates effects relative
to the first level of each attribute and calculates standard errors clustered
by persona.
